...
## Declarative Animation Props Summary

Framer Motion provides a comprehensive set of declarative animation props organized into several categories:
...
- **transition**: The default transition for this component to use when an animation prop (animate, whileHover etc) has no transition defined
- **variants**: The variants for this component
- **style**: The normal React DOM style prop, with added support for motion values and independent transforms
- **layout**: If true, this component will perform layout animations
- **layoutId**: If set, this component will animate changes to its layout
- **layoutDependency**: Reduce layout measurements by dependency
- **layoutScroll**: Measure scroll offset for layout animations
- **layoutRoot**: Mark element as position: fixed for layout animations

### Gesture Animation Props
- **whileHover**: Hover animation state or variant
- **whileTap**: Tap/press animation state or variant
- **whileFocus**: Focus animation state or variant
- **whileDrag**: Drag animation state or variant
- **whileInView**: Animation while element is in viewport

### Event Callback Props
- **onUpdate**: Callback triggered every frame
- **onAnimationStart**: Callback when animation starts
- **onAnimationComplete**: Callback when animation completes
- **onHoverStart**: Callback when pointer starts hovering
- **onHoverEnd**: Callback when pointer stops hovering
- **onTapStart**: Callback when pointer starts pressing
- **onTap**: Callback when pointer stops pressing inside component
- **onTapCancel**: Callback when pointer stops pressing outside component
- **onPan**: Callback when pan gesture is recognized
- **onPanStart**: Callback when pan gesture starts
- **onPanEnd**: Callback when pan gesture ends
- **onDrag**: Callback when drag gesture is recognized
- **onDragStart**: Callback when drag gesture starts
- **onDragEnd**: Callback when drag gesture ends
- **onDirectionLock**: Callback when drag direction is determined
- **onViewportEnter**: Callback when element enters viewport
- **onViewportLeave**: Callback when element leaves viewport
- **onLayoutAnimationStart**: Callback when layout animation starts
- **onLayoutAnimationComplete**: Callback when layout animation completes

### Drag Control Props
- **drag**: Enable dragging (true, "x", or "y")
- **dragConstraints**: Apply constraints on draggable area
- **dragSnapToOrigin**: Animate back to center when released
- **dragElastic**: Degree of movement outside constraints
- **dragMomentum**: Apply momentum from pan gesture
- **dragTransition**: Customize dragging momentum transition
- **dragDirectionLock**: Lock drag direction
- **dragPropagation**: Allow drag propagation to children
- **dragControls**: Control dragging from external components
- **dragListener**: Determine drag gesture trigger source

### Advanced Props
- **inherit**: Prevent inheriting parent variant changes
- **custom**: Custom data for dynamic variants
- **transformTemplate**: Customize transform string generation
