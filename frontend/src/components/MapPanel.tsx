...
const MapContent: FC<{
  ...
  const adjustedMapContainerStyle = {
    ...
    boxShadow:
      theme === 'dark'
        ...
        : mapContainerStyle.boxShadow,
    backgroundColor:
      theme === 'dark' ? 'rgba(0,0,0,1)' : mapContainerStyle.backgroundColor,
  };
  const inputStyle = {
    padding: '12px 16px',
    borderRadius: '4px',
  ...
  const [pendingZoomTarget, setPendingZoomTarget] = useState<{
    lat: number;
    lng: number;
    zoom: number;
  } | null>(null);
  ...
  const [searchResultMessage, setSearchResultMessage] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (pendingZoomTarget && mapInstance) {
      mapInstance.setView(
        [pendingZoomTarget.lat, pendingZoomTarget.lng],
        pendingZoomTarget.zoom,
        { animate: false },
      );
